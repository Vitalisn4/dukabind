...
If a required field is NULL or missing, we refuse. The LLM must not invent
amounts. Cashier messages are localized (English / French / Swahili) and the
deterministic message is authoritative in every language.
"""
...
def refuse_not_found(lang: str, what: str) -> BinderResult:
    ...
    ``sku``, ``quantity``, ``unit_price``), which is localized, or a
    user-supplied ledger name, which is preserved verbatim.
    """
