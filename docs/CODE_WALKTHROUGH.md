...
**Product thesis:** change a ledger row and the answer must change. Delete a required field and the system refuses. The model is instructed not to invent amounts; it is never trusted to choose SQL.
...
**Keep in sync:** `app/db/fixture.py` (nicknames + normalize helpers) with `app/db/seed.sql` (rows).
...
| `seed_duka_b.sql` | Loads Marché Nkolmébé (`duka_b`) rows, the second shop for the held-out eval. |
...
| `allowlist.py` | Finite map of named SQL; unknown name or missing params raise `ValueError`; never concatenates user text into SQL (**C1/C2**). |
| `refuse.py` | Refuse strings + `credit_decision` arithmetic in Python (**C4**). |
| `citations.py` | Rows become compact JSON for narration prompts. |
| `pipeline.py` | `handle_ask(conn, text)`, the full answer path **without** a model. |
...
| `scripts/run_profiler_smoke.sh` | `adtc-profiler` participant mode, writes `benchmarks/raw/submission.json`; optional `--full`. |
...
| `scripts/thread_matrix.sh` | `llama-bench` thread bake-off (`-t 2,3,4,6,8`) + temp sampling, output in `benchmarks/raw/`. |
| `scripts/thermal_soak.sh` | Sustained generation soak (default 10 min) sampling package temp every 5 s, producing CSV + PASS/FAIL. |
...
```bash
bash scripts/thread_matrix.sh   # llama-bench thread bake-off, output in benchmarks/raw/thread_matrix_*.{jsonl,md}
bash scripts/thermal_soak.sh    # 10-min sustained soak, output in benchmarks/raw/thermal_soak_*.csv + PASS/FAIL
bash scripts/run_profiler_smoke.sh  # raw JSON, output in benchmarks/raw/submission.json (gitignored)
bash scripts/ram_capped_proof.sh    # 8 GB-class proof: full stack under cgroup MemoryMax cap, produces headroom report
```
...
Runs on every push/PR: `pip install -r requirements.txt`, then `pytest` (77 tests), `ruff`, the `static_analysis.sh` gate (ruff + bandit + shellcheck), the held-out eval (31 checks), then the **held-out report freshness gate** (regenerates `evals/heldout/REPORT.md` via `--write-report` and fails if it drifts from the committed artifact; the daily `Generated:` date is excluded from the comparison), then the offline binder proof. Contest-claims drift is caught by `tests/test_metadata.py`.
