...
- `CONTRIBUTING.md`, a fresh-machine reproduction runbook written for an auditor/judge.
- `.github/workflows/link-health.yml`, a weekly + manual GGUF URL-rot guard (HEAD probe
...
- `README.md`, a professional judge-facing rewrite: pitch, “why not a chatbot”
...
- Demo assets re-rendered for the multilingual track: `demo/demo.mp4` is now
  108 s (within the two-minute gate) with a French and Swahili binder scene,
  and `demo/screenshots/` grew to 6 numbered stills including
  `06-multilingual.png`; the `README.md` demo section matches.
- Editorial pass across docs and code comments: prose rewritten in plain
  human voice with standard punctuation (no em-dashes, arrows, or emoji),
  demo captions updated to the same style.
...
- `docs/FREEZE_CHECKLIST.md`, the gate-freeze checklist (T1–T3, T5, T8–T9, T11, T13).
- `benchmarks/submission.json`, the freeze snapshot of the measured adtc-profiler
...
- Fail-closed refusal when a customer row has `outstanding IS NULL`
  ...
  Unit + end-to-end pipeline tests (suite 44 to 46). (PR #5.)
